...
def logging_csv(number, mode, landmark_list, point_history_list):
    ...
    if mode == 1 and (0 <= number <= 9):
        csv_path = 'hand_gesture_recognition/model/keypoint_classifier/temppoint.csv'
        with open(csv_path, 'a', newline="") as f:
            writer = csv.writer(f)
            writer.writerow([number, *landmark_list])
    if mode == 2 and (0 <= number <= 9):
        csv_path = 'hand_gesture_recognition/model/point_history_classifier/point_history.csv'
        with open(csv_path, 'a', newline="") as f:
            writer = csv.writer(f)
...
def draw_info_text(image, brect, handedness, hand_sign_text,
    ...
    if True:
        cv.putText(image, "Press Esc to Quit", (10, 60),
